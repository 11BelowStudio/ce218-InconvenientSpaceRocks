...
public class EnemyController extends ControllerAdapter{
    ...
    public void setEnemyShip(EnemyShip e){ enemyShip = e; }
    ...
    private void nextUpdateIn(){ framesUntilUpdatingEnemyPosition = (int)(Math.random() * MAX_POS_UPDATE_DELAY); }
    ...
    private boolean thrustIfSlowerThan(double minSpeed){
        //will turn the thrust on if the magnitude of velocity is below minSpeed.
        boolean nowThrusting;
        if (nowThrusting = (enemyShip.velocity.mag() < minSpeed)){
            action.thrust = 1;
        } else {
            action.thrust = 0;
        }
        return nowThrusting; //returns whether or not it's now thrusting
    }
    ...
    private boolean analysePosition(){
        ...
        Vector2D vectorBetween = enemyShip.getPosition().getVectorTo(targetPosition, FRAME_WIDTH, FRAME_HEIGHT);
        //Vector2D vectorBetween = Vector2D.g
        distanceBetween = vectorBetween.mag();
        angleDifference = vectorBetween.angle(enemyShip.getDirection());
        return true;
    }
    ...
    private void randomShoot(double shootChance){ action.shoot = (Math.random() < shootChance); }
